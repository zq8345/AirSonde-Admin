...
## ⚠️ 改按钮样式之前，先查 `button.danger:not(.linkish)`（2026-09-05 一天撞三次）

`public/style.css` 里有一条：

```css
button.danger:not(.linkish) { … padding: 8px 12px; margin-left: auto; … }
```

它的特异性是 **(0,2,1)**，会压过绝大多数**类选择器组合**（`.foo .danger` 只有 (0,2,0)）。
2026-09-05 一天之内，我写的三条规则被它压掉，**三次都没有报错、没有变红**：

| 我写的 | 我以为 | 实际 |
|---|---|---|
| 分类页行高 | 「改名」「删除」等高 | 「删除」拿到 8/12 的 padding ⇒ 高 37.5 vs 23.5，**行被撑到 50.5** |
| `.cdlg-foot .danger { margin-left: 0 }` | 两个键靠右并排 | `margin-left:auto` 仍在，computed **314px** |
| （同上）注释 | 描述的是 flex-end 的效果 | 描述的那件事**从来没发生过** |

**⇒ 动任何带 `.danger` 的按钮之前**：
1. 在浏览器里**问它"哪条规则在控制这个属性"**（遍历 `document.styleSheets` 找 `el.matches(sel)`），
   ⛔ 不要读 CSS 猜 —— 三次里有两次我猜的是另一条规则（`.linkish.danger`，那条只设了 color）。
2. 要盖过它，选择器至少要有 **(0,2,1)**（例如再加一个元素名或 `:not()`），
   ⛔ 或者干脆别盖：**先确认那个"默认长相"是不是本来就更好**
   （删除确认框那次就是：它把「删除」推到右端、与「取消」隔开，对破坏性操作反而更合适）。
3. 🔴 **决定不盖的时候，把那条没生效的规则删掉。**
   一条什么都没做、而注释说它做了某事的规则，比没有这条规则更坏 —— 下一个人会照着注释理解布局。

⚠️ 同一族的更一般形式：**未定义的自定义属性也不报错**。
`--tile` 在 967 行被改成"由列宽出"之后再没定义过，而 `.icard` / `.iadd` 上的
`width: var(--tile)` / `height: var(--tile)` 就一直是死的（invalid-at-computed-value-time ⇒ 退回 auto）。
症状只在**空态**露出来：新建页那个添加格塌成 166×24 的一条。
⇒ **CSS 不像 JS，写错通常不会响。判"这条规则生效了没有"只能落到 computed style 上。**

## ⚠️ `appendMd` 只认 `**粗体**` —— 别的标记会原样印在屏幕上（2026-09-05 一天撞三次）

`public/app.js` 的 `appendMd(node, text)` 就一条规则：

```js
String(text).split(/\*\*(.+?)\*\*/g)   // 只有 ** ** 变 <b>，其余原样进 textNode
```

**它不认反引号、不认 `·` 之外的任何 markdown、不认换行。** 一天之内我在同一族上栽了三次：

| 哪里 | 我写的 | 屏幕上出现的 |
|---|---|---|
| 分类页悬空引用提示 | `el("div", null, "**本来就这样**…")` | `**本来就这样**`（星号原样） |
| 删除确认框正文 | 同上 | 同上 |
| 删除成功提示 | `` `commit \`${sha7}\`` `` | `commit \`abc1234\``（反引号原样） |

**⇒ 三条硬规矩：**
1. 任何要出现 `**粗体**` 的地方，**必须走 `appendMd(node, text)`**，
   ⛔ 不能写 `el("div", cls, text)` —— 后者是 `textContent`，星号会照印。
2. **除了 `**粗体**`，别写任何标记。** 要等宽就 `el("code", null, x)`，
   要换行就多来一个 `<div>` —— ⛔ `
` 在 `.notice` 里也不换行（那一族没有 `pre-wrap`）。
3. **判据落在渲染结果上**：`node.querySelectorAll("b")` 有没有东西、
   `innerText` 里还有没有 `*` 或反引号。⛔ 不是"我写了 `**` 所以它是粗的"。

⚠️ 这与上一节（`.danger` 特异性 / 未定义的自定义属性）是**同一个性质的坑**：
**写错的那一刻什么都不会响** —— 不报错、不变红、不进控制台。
⇒ 判"它生效了没有"只能落到 **computed style / 真实渲染出来的 DOM** 上，
⛔ 落不到源码那一行上。
...
#### 🔴 2026-09-04：这条**记在这里也照样又踩了一次**，所以它现在进闸了

```bash
npm run typecheck 2>&1 | tail -3 && echo "exit=$?"   # → 打印 exit=0
```
而 tsc 实际有一条真错（`noUncheckedIndexedAccess` 下 `items[i]` 是 `T | undefined`）。
错误就打在屏幕上，判据没在读它 —— **和 ① 一模一样的形状，写在 README 里的提醒没拦住。**

⇒ **光记下来不够，判据得进闸。** 修法不是"提醒大家取真退出码"（那还是靠人记），
是让**判词永远是输出的最后一行**（`scripts/gate.mjs`）：

```bash
npm run gate 2>&1 | tail -1     # ✅ GATE: GREEN  /  GATE: RED — typecheck exit N
```

**判据 = 最后一行必须是 `GATE: GREEN`。⚠️ 这条穿得过管道，而 `$?` 穿不过。**
⛔ 没有 `GATE:` 那一行 = 没跑完 = 当红，别当成"大概过了"。

⚠️ 写这个包装时**当场又栽了一次，值得一起记**：第一版忘了 `shell: true`，
Windows 上 Node 拒绝直接 spawn `npm.cmd` ⇒ 子进程根本没启动，却打印了 `GATE: RED`。
那是一次**假红** —— 它报的不是"检查没过"，是"检查没跑起来"。两种红现在分开说：
一种要改代码，另一种要修这台机器，混在一起时人会去改错的那一边。
（✅ 失败方向当时是对的：跑不起来落到红，不是落到绿。）
